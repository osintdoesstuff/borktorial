...
namespace borktorial
{
    internal class errgen
    {
        ...
        static string[] templates = [
            "<animal>_BROKE_CONTAINMENT",
            "CRITICAL_PROCESS_FAILED<notem>",
            "SCP-<scpnum>_BROKE_CONTAINMENT",
            ...
            "<subject>_ERROR_<num>",
            "<object>_ERROR_<num>",
            "KERNEL_ERROR_<lowerltr><num16>",
            "SYS_UNKNOWN_ERR_<lowerltr><num16><hletter><nibble>",
            "INVALID_VALUE_IN_<register>",
            "MEMORY_ERROR_<addr386>",
            "<animal>_<verb>_<object>",
            "<subject>_<object>_ERROR",
            "<subject>_<verb>_<object>",
            "<animal>_<verb>_<object>",
            "<subject>_<verb>_<animal>",
            "<kerbal>_<verb>_<object>"
            ];
        ...
        public static string[] Generate()
        {
            ...
            gErrI = gErrI.Replace("<notem>", ""); // Marker to designate "NOt TEMplate"
            ...
            File.AppendAllText("CRASH.LOG", $"A call to errgen was made at {DateTime.Now} >> {gErrI} in {processName}!\r\n");
            return [gErrI, processName];
        }
        public static string sf15(int length, int gs) {
            string crashSignature = "";
            for (int i = 0; i < length; i++)
            {
                if(gs >= 1)
                {
                    if (i > 0 && i % gs == 0) { crashSignature += "-"; }
                }
                crashSignature += hexDigits[rand.Next(hexDigits.Length)].ToString();
            }
